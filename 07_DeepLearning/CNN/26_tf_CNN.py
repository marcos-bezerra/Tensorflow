...
########################################## Dropout
# Aplicamos o dropout para reduzir o overfitting. O dropout vai eliminar
# algumas unidades (nas camadas ocultas, de entrada e de saída) na rede neural.
# A decisão sobre qual neurônio será eliminado é randômica e aplicamos uma 
# probabilidade para isso. Esse parâmetro pode ser ajustado para otimizar o desempenho da rede.
...
# converte o valor de x para o formato de tensor numa dimensão específica de 28 x 28
...
def conv2d(img, w, b): # camada de convolução
...
def max_pool(img, k): # camada de pooling
...
wc1 = tf.Variable(tf.random.normal([5, 5, 1, 32])) # 5x5 convolução, 1 input, 32 outputs
wc2 = tf.Variable(tf.random.normal([5, 5, 32, 64])) # 5x5 convolução, 32 inputs, 64 outputs
wd1 = tf.Variable(tf.random.normal([4*4*64, 1024])) # camada densa, fully connected, 4*4*64 inputs, 1024 outputs
